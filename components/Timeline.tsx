...
import { ResponsiveContainer, ScatterChart, Scatter, XAxis, YAxis, ZAxis, Cell } from 'recharts';
...
export const Timeline: React.FC<TimelineProps> = ({ paintings, onSelect }) => {
  ...
  return (
    <div className="w-full bg-white/10 dark:bg-slate-800/20 rounded-3xl p-1 border border-white/30 dark:border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.05)] backdrop-blur-3xl transition-all duration-500 relative">
      ...
      {/* Scrollable Area */}
      <div
        className="w-full overflow-x-auto pb-6 px-4 scrollbar-thin scrollbar-thumb-blue-500/20 scrollbar-track-transparent"
        ref={containerRef}
        ...
        <div style={{ width: minChartWidth, height: '320px' }} className="relative">
            ...
            <ResponsiveContainer width="100%" height="100%">
                ...
                <Scatter
                  name="Paintings"
                  data={data}
                  onClick={(_, index) => onSelect(data[index] as Painting)}
      ...
      {hoveredData && ReactDOM.createPortal(
        <div
          ...
          style={{ left: mousePos.x + 16, top: mousePos.y - 120, zIndex: 9999 }}
        >
          <div className="bg-white/95 dark:bg-slate-900/95 border border-white/40 dark:border-white/10 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.25)] text-sm backdrop-blur-xl w-[260px] overflow-hidden">
            <div className="w-full h-32 bg-slate-200 dark:bg-slate-800 overflow-hidden">
              <img src={hoveredData.imageUrl} alt={hoveredData.title} className="w-full h-full object-cover" />
            </div>
            <div className="p-4">
              <p className="font-bold text-slate-900 dark:text-white font-serif text-lg mb-1 leading-tight">{hoveredData.title}</p>
              <div className="flex items-center gap-2 mt-2">
                <span className="px-2 py-0.5 rounded bg-blue-100/50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-bold">{hoveredData.year}</span>
                <span className="text-slate-500 dark:text-slate-400 text-xs">Score: {hoveredData.aestheticScore}</span>
              </div>
            </div>
          </div>
        </div>,
